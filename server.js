...
async function getAISignal(symbol, accountId) {
    try {
        ...
        // Buy signals - more aggressive for high-speed trading
        if (rsi < 35) {
            action = 'BUY';
            confidence = 0.85;
        ...
        // If still HOLD, try to follow trend with lower confidence
        if (action === 'HOLD') {
...
async function shouldClosePosition(position, accountId) {
    try {
        ...
        if (profitPercent > 0) {
            // Take profit more aggressively for high-speed trading
            if (profitPercent >= 2) {
                shouldClose = true;
...
class HalalTradingEngine {
    ...
    async start() {
        ...
        // Analysis EVERY SECOND for high-speed trading
        ...
        // Monitor positions every 1 second as well
